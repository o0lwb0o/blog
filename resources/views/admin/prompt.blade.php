...
<div class="error-container">
    <h1> 信息提示 </h1>
    <div class="errorcon">
        @if (session('list.code') == 1)
        <i class="icon-smile-o"></i>{{ session('list.smg') }}
        @else
        <span><i class="icon-frown-o"></i>{{ session('list.msg') }}</span>
        @endif
    </div>
    <h4 class="smaller">页面自动 <a id="href" href=" {{ session('list.url') }}">跳转</a> 等待时间： <b id="wait">@if (session('list.date'))  {{session('list.date')}}@else 3 @endif</b></h4>
